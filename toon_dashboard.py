...
def run_dashboard():
    ...
    if os.path.exists(registry_path):
        with open(registry_path, "r") as f:
            ...
            print(f"{'SYMBOL':<12} {'LANE':<6} {'STATUS':<10} {'VARIANT':<8} {'SHARPE':<8} {'PROFIT.F':<8} {'TRADES'}")
            print("-" * 75)
            for key, details in sorted(sleeves.items()):
                ...
                print(f"{sym:<12} {lane:<6} {status:<10} {str(variant):<8} {s_str:<8} {pf_str:<8} {trds}")
    else:
        print("\n>> No Portfolio Registry found.")

    print(f"\n[ 2. LIVE ML TARGET RADAR (LATEST META-VERDICTS) ]")
    print(f"{'SYMBOL':<12} {'WINNING STRATEGY':<20} {'EDGE (SHARPE)':<15} {'DIRECTION':<10} {'SIGNAL':<12} {'CONF'}")
    print("-" * 88)
    ...
    for vf in verdict_files:
        try:
            with open(vf, "rb") as f:
                ...
                print(f"{sym:<12} {winner:<20} {sharpe_str:<15} {direction:<10} {strength:<12} {conf_str}")
        except Exception:
            pass

    print(f"\n[ 3. REAL-TIME INFERENCE TARGETS (TP / SL) ]")
    print(f"{'SYMBOL':<12} {'LANE':<6} {'DIRECTION':<10} {'SIGNAL':<12} {'STOP LOSS':<15} {'TAKE PROFIT (1)':<18} {'TAKE PROFIT (2)':<18}")
    print("-" * 105)
    ...
    for sf in signal_files:
        try:
            with open(sf, "r") as jf:
                ...
                print(f"{sym:<12} {lane:<6} {direction:<10} {strength:<12} {sl_str:<15} {tp1_str:<18} {tp2_str:<18}")
        except Exception:
            pass
