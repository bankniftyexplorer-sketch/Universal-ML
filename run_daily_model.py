...
def run_backtest(
    df, prob_array, initial_capital=10000.0, risk_pct=0.02, conf_threshold=0.60
):
    ...
    for i in range(len(df) - 1):
        ...
        if position is not None:
            ...
            if p_type == "LONG":
                if next_low <= p_sl:
                    ...
                    pnl = (
                        ...
                        * position["size"]
                        * (0.5 if position["tp1_hit"] else 1.0)
                    )
                    position["pnl"] += pnl
                    trade_closed = True
            ...
            if confidence >= conf_threshold:
                ...
                if sl_dist > 0:
                    slippage_bps = 0.0003
                    entry_price = (
                        float(next_open) * (1 + slippage_bps)
                        if direction == "LONG"
                        else float(next_open) * (1 - slippage_bps)
                    )
        ...
        if position is not None:
            if position["type"] == "LONG":
                unrealized = (
                    (next_close - position["entry_price"])
                    * position["size"]
                    * (0.5 if position["tp1_hit"] else 1.0)
                )
            else:
                unrealized = (
                    (position["entry_price"] - next_close)
                    * position["size"]
                    * (0.5 if position["tp1_hit"] else 1.0)
                )
            if position["tp1_hit"]:
...
def main():
    ...
    merged = holographic_feature_engine(
        df_1h=df_1d_labelled, df_1d=None, df_1w=df_1w.copy(), df_1m=None
    )
    ...
    merged["rel_strength_w"] = (merged["close"] - merged["w_close"]) / (
        merged["w_close"] + 1e-9
    )
    ...
    merged["target"] = np.where(
        _ret_rdm > 0.001, 1, np.where(_ret_rdm < -0.001, 0, np.nan)
    )
    ...
    wf = walk_forward(
        ...
        feature_cols,
        n_splits=5,
        min_train_bars=MIN_TRAIN_BARS_1D,
    ...
    backtest_probs = np.array(
        [oos_proba_map[pd.Timestamp(t)] for t in backtest_df["time"]]
    )
    ...
    ax1.fill_between(
        res["t_c"], res["eq_c"], min(res["eq_c"]) * 0.99, color="#00d4ff", alpha=0.1
    )
    ax1.set_title(
        f"{SYMBOL.upper()} 1D-MODEL Backtest Equity Curve (Honest OOS)",
        color="white",
        fontsize=16,
        fontweight="bold",
    )
