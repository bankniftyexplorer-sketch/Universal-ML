...
def _build_prob_array(df_strategy: pd.DataFrame, oos_proba_map: dict) -> np.ndarray:
    if not isinstance(df_strategy, pd.DataFrame) or df_strategy.empty or 'time' not in df_strategy.columns:
        return np.full(0, np.nan, dtype=float)
    return np.array(
        [float(oos_proba_map.get(pd.Timestamp(t), np.nan)) for t in df_strategy['time']],
        dtype=float,
    )
...
def run_strategy_zoo(
    ...
    **kwargs,
    ...
    walk_forward_fn = kwargs.pop('walk_forward_fn', walk_forward)
    symbol = kwargs.pop('symbol', '')

    if kwargs:
        raise TypeError(f"Unexpected keyword arguments: {sorted(kwargs)}")

    feature_cols_arg = None
    if args:
        first = args[0]
        if isinstance(first, list):
            feature_cols_arg = list(first)
        elif callable(first):
            walk_forward_fn = first
        elif len(args) >= 4 and isinstance(args[3], str):
            symbol = args[3]

    if len(args) >= 2 and callable(args[1]):
        walk_forward_fn = args[1]

    print(f"  [Meta] Strategy zoo starting for {symbol or 'UNKNOWN'} with MODEL_N_JOBS={MODEL_N_JOBS}")
    zoo_results: dict[str, dict] = {}

    original_atr_mult = ume.BARRIER_ATR_MULT
    original_edge_r = ume.MIN_LABEL_EDGE_R
    original_best_r = ume.MIN_LABEL_BEST_R

    for key, strategy in STRATEGY_ZOO.items():
        atr_mult = float(strategy["atr_mult"])
        min_edge_r = float(strategy["min_edge_r"])
        print(f"  [Meta] [Zoo] Running {key} - ATR_MULT={atr_mult:.2f} MIN_EDGE_R={min_edge_r:.2f} ...")

        df_copy = df_model_ready.copy(deep=True)

        try:
            ume.BARRIER_ATR_MULT = atr_mult
            ume.MIN_LABEL_EDGE_R = min_edge_r
            ume.MIN_LABEL_BEST_R = min_edge_r * 0.6
            ...
            ume.MIN_LABEL_EDGE_R = original_edge_r
            ume.MIN_LABEL_BEST_R = original_best_r
        ...
        if feature_cols_arg:
            all_holo_cols = [c for c in feature_cols_arg if c in df_copy.columns]
        ...
        if not wf_result:
            wf_result = {'oos_proba_map': {}, 'feature_cols': feature_cols, 'df': df_copy}
        else:
            ...
            if isinstance(strategy_df, pd.DataFrame) and not strategy_df.empty:
                ...
                try:
                    ...
                    print(f"  [Meta] [Zoo] Trade-plan model training failed for {key}: {exc}")
...
def meta_select_strategy(
    ...
    for key in STRATEGY_ZOO:
        ...
        if not isinstance(strategy_df, pd.DataFrame) or strategy_df.empty or not feature_cols or not oos_proba_map:
            metrics[key] = {
        ...
        for conf_threshold in EXECUTION_CONF_GRID:
            ...
            total_return_pct = (
                ...
                if np.isfinite(final_equity) else np.nan
            )
            ...
            if best_exec is None or _candidate_rank(candidate) > _candidate_rank(best_exec):
                best_exec = candidate
    ...
    if not candidates:
        ...
        candidates.sort(
            key=lambda item: (
                ...
                -(item[1]['max_drawdown'] if np.isfinite(item[1]['max_drawdown']) else np.inf),
            ),
            reverse=True,
        )
        ...
        if len(candidates) > 1:
            ...
            equity_gap_pct = abs(winner_metrics['total_return_pct'] - runner_metrics['total_return_pct'])
...
def verdict_output(
    ...
    last_row = winning_df.iloc[-1].copy() if isinstance(winning_df, pd.DataFrame) and not winning_df.empty else pd.Series(dtype=float)

    if model is not None and feature_cols and not last_row.empty:
        pred = predict_next_bar(model, feature_cols, last_row, LIVE_CONFIDENCE_THRESHOLD)
    else:
        pred = {
            'direction': 'ERROR',
            'prob_up': np.nan,
    ...
    confidence_text = f"{float(confidence) * 100:.1f}%" if confidence is not None and np.isfinite(confidence) else "n/a"
    ...
    print(f"  Exec Conf  : {_fmt_float(metrics.get(winner_key, {}).get('conf_threshold', np.nan)).strip()}")
    print("======================================================================")
...
if __name__ == '__main__':
    ...
    zoo_results = run_strategy_zoo(df_full.copy(), df_1d, df_1w, df_1m, symbol)
    ...
    conf_text = (
        ...
        if np.isfinite(winner_signal.get('confidence', np.nan)) else 'n/a'
    )
    top_text = (
        f"==============================================================\n"
    ...
    return_vals = [metrics.get(key, {}).get('total_return_pct', np.nan) for key in strategies]
    win_rates = [metrics.get(key, {}).get('win_rate', np.nan) * 100.0 for key in strategies]
    ...
    ax_bottom.set_title('OOS Trading Return by Meta Strategy', color='white', fontsize=16, fontweight='bold')
    ...
    for rect, key, return_val, win_rate in zip(bars, strategies, return_vals, win_rates):
    ...
    fig.suptitle(f'{symbol.upper()} Meta Strategy Verdict', color='white', fontsize=18, fontweight='bold', y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(report_path, dpi=120, facecolor=fig.get_facecolor(), bbox_inches='tight')
    plt.close(fig)
