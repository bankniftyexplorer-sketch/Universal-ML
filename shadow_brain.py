...
class ShadowBrain:
    ...
    def _build_model(self):
        return lgb.LGBMClassifier(
            ...
            random_state=42, n_jobs=4, verbose=-1, objective='binary'
        )
    ...
    def train(self, days=10000):
        ...
        if df.empty or len(df) < 15:
            print("  [SHADOW] Insufficient trade records for meta-modeling. Bypass engaged.")
            return False
        ...
        if len(df) < 15 or df['win_loss_target'].nunique() < 2:
            print("  [SHADOW] Target requires both wins and losses (>15). Bypass engaged.")
            return False

        # Build feature set dynamically from joined OHLCV
        base_exclusions = ['timestamp', 'base_symbol', 'direction', 'conf_score', 
        ...
        try:
            ...
            print(f"  [SHADOW] Meta-model trained on {len(df)} records. Features: {len(self.feature_cols)}")
            return True
        except Exception as e:
